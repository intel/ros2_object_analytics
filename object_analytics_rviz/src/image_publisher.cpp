...
public:
  ...
  : Node("image_publisher")
  {
    ...
    tra_subscription_ = this->create_subscription<TrackingMsg>(
      kTopicTracking_, rclcpp::ServicesQoS(),
      std::bind(&ImagePublisher::tra_callback, this, _1));
  }
...
private:
  ...
  using FilteredSync = message_filters::TimeSynchronizer<ImageMsg, TrackingMsg>;
    ...
    std::vector<TrackingObjectInBox> tra_objects)
  {
    ...
    for (auto tra : tra_objects) {
      ...
      if (roi.x_offset != 0 && roi.y_offset != 0 && roi.width != 0 &&
        roi.height != 0)
      {
  ...
  void drawObject(
    cv_bridge::CvImagePtr & cv_ptr, ObjectRoi roi,
    std::string obj_name, int32_t obj_id)
  {
    RCLCPP_DEBUG(this->get_logger(),
      "Draw: name=%s, id=%d, roi(%d,%d,%d,%d), img(%d,%d)",
    ...
    cv::Point middle_left =
      cv::Point(roi.x_offset, (roi.y_offset + roi.height / 2));
    cv::Point top_right =
      cv::Point(roi.x_offset + roi.width, roi.y_offset + roi.height);
    cv::rectangle(cv_ptr->image, bottom_left, top_right,
    ...
    ss_roi << "ROI[" << roi.x_offset << "," << roi.y_offset << "," << roi.width <<
      "," << roi.height << "]";
    cv::putText(cv_ptr->image, ss_roi.str(), bottom_left,
      cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 255, 0), 2);

    // Draw object name and tracking id together in the middle left of the
    // rectangle
    std::stringstream ss_name_id;
    ss_name_id << obj_name << "(#" << obj_id << ")";
    cv::putText(cv_ptr->image, ss_name_id.str(), middle_left,
      cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 255, 0), 2);
    ...
    cv::putText(cv_ptr->image, ss_tra, cvPoint(2, 30), cv::FONT_HERSHEY_SIMPLEX,
