...
class KalmanFilter {
  ...
  */
  KalmanFilter(int dynamDim, int measureDim, int controlParams = 0,
               int type = CV_32F);
  ...
  @param type Type of the created matrices that should be CV_32F or CV_64F.
   */
  void init(int dynamDim, int measureDim, int controlParams = 0,
            int type = CV_32F);
   ...
   */
  const cv::Mat& correct(const std::vector<cv::Mat>& measurements,
