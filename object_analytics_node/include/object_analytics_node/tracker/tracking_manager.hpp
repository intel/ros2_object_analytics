 ...
 *
 * TrackingManager owns a list of trackings. A tracking is added to the list when it is initially
 * detected, see @ref detect(). Then in the successive frames, every tracking is updated
 * independently, see @ref track().
 *
 * For an object detected, TrackingManager search the existing list if it is tracked already.
 * A detected object has its own object name (e.g. person, dog, etc.) and a roi (region of
 * interest, also known as bounding box). The roi's matching level is measured by its overlapping
 * rate and its centor deviation, see @ref model::ObjectUtils::getMatch(). A @ref kMatchThreshold
 * is used as the minimum matching level of roi. By matching the object name and the roi, see
 * @ref getTracking(), TrackingManager find an existing tracking for this object.
 * Or a new tracking should be added, see @ref addTracking().
 *
 * TrackingManager maintains a @ref tracking_cnt. When adding a new tracking, the value of
 * tracking_cnt will be assigned to that tracking, as a unique ID across frames. Then the
 * tracking_cnt automatically increased by one.
 *
 * TrackingManager also maintains a @ref kProbabilityThreshold, only when detected with a
 * confidence level not less than this threshold will the object be added to the tracking
 * list. This is necessary to mask any unexpected or unstable detection results.
 *
 * Paralleling computation is enabled in TrackingManager, supported by openmp from compilers.
 * @ref kNumOfThread specifies the number of threads used for paralleling computation.
 * Usually this should be less than the maximum number of threads supported by the platform.
 */
class TrackingManager
{
public:
   ...
   *
   * For each object detected, TrackingManager will search the list if this object has been tracked
   * already. This is done by @ref getTracking(). If tracking does not exist for this object,
   * a new tracking will be added.
   *
   * Only when detected with a confidence level not less than @ref kProbabilityThreshold will the
   * object be marked as "Detected" in this function, see @ref Tracking::setDetected().
   *
   ...
   */
  void detect(const cv::Mat & mat, const object_msgs::msg::ObjectsInBoxes::ConstSharedPtr & objs);

  /**
   * @brief Manage trackings when a new frame arrives.
   *
   * When a new frame arrives, for all existing trackings, TrackingManager will update their
...
private:
   ...
   *
   * The current value of @ref tracking_cnt will be assigned to this new tracking,
   * which denotes a unique ID of the same object across frames.
   * Then the tracking_cnt automatically increases by one.
   *
  ...
  std::shared_ptr<Tracking> addTracking(
    const std::string & obj_name, const float & probability, const cv::Rect2d & rect);
   ...
   *
   * For a detected object, this function returns the most matching tracking, with the same object
   * name, and the most matching roi measured by @ref model::ObjectUtils::getMatch(),
   * not less than @ref kMatchThreshold.
   *
   * @param[in] obj_name Name of the object (e.g. people, dog, etc.).
   ...
   */
  bool validateROI(const cv::Mat & mat, const sensor_msgs::msg::RegionOfInterest & droi);
};
}  // namespace tracker
}  // namespace object_analytics_node
