...
#ifndef OBJECT_ANALYTICS_NODE_TRACKER_TRACKING_NODE_H
#define OBJECT_ANALYTICS_NODE_TRACKER_TRACKING_NODE_H
 ...
 * arrives.
 *
 * TrackingNode simply buffers the RGB image in @ref rgb_cb(), and waiting for the arrival of the detection frame.
 * Then in @ref obj_cb(), TrackingNode noted down this object frame and last object frame. The "last" object frame is
 * used to initiate trackings, then TrackingNode will process all successive frames before "this" object frame.
 *
 * TrackingNode has a @ref TrackingManager to process tracking updates from both detection frames and tracking
 * frames.
 */
...
private:
   ...
   *
   * @param[in] objs List of objects detected in a detection frame.
   * @return true if new object apprear in detection or tracked result not precision enough.
   */
  bool check_rectify(const object_msgs::msg::ObjectsInBoxes::ConstSharedPtr& objs);


  rclcpp::Publisher<object_analytics_msgs::msg::TrackedObjects>::SharedPtr pub_tracking_; /**< Tracking publisher.*/
  rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr sub_rgb_;                      /**< Rgb image subscriber.*/
  rclcpp::Subscription<object_msgs::msg::ObjectsInBoxes>::SharedPtr sub_obj_; /**< Object detection subscriber.*/
  std::unique_ptr<TrackingManager> tm_;                                       /**< TrackingManager*/
  std::vector<sensor_msgs::msg::Image::ConstSharedPtr> rgbs_;                 /**< Rgb image buffer.*/
  std::vector<object_analytics_msgs::msg::TrackedObjects::SharedPtr> tracks_; /**< tracked objs records.*/
  object_msgs::msg::ObjectsInBoxes::ConstSharedPtr last_obj_,
      this_obj_;                                                  /**< Last detection frame, and this detection frame.*/
  builtin_interfaces::msg::Time last_detection_, this_detection_; /**< Timestamp of last and this detection frame.*/
  int32_t kRgbQueueSize;
};
}  // namespace tracker
}  // namespace object_analytics_node
#endif  // OBJECT_ANALYTICS_NODE_TRACKER_TRACKING_NODE_H
