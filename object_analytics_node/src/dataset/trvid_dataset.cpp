...
void vidDataset::load(const std::string & rootPath)
{
  ...
  if (namesList.is_open()) {
    ...
    while (getline(namesList, datasetName)) {
      ...
      if (currDatasetID == 0) {
        TRACE_INFO("Video Dataset Initialization...\n");
      }

      std::string fullPath =
        rootPath + "/" + datasetName + "/data/" + datasetName + ".webm";
      if (!fileExists(fullPath)) {
        TRACE_INFO("vid(%s) file is not exist\n", fullPath.c_str());
        continue;
      }
    ...
    TRACE_INFO("Couldn't find a *list.txt* in video dataset!!!");
  }
...
bool vidDataset::initDataset(std::string dsName)
{
  ...
  if (id > 0 && id <= static_cast<int>(data.size())) {
    ...
    if (!cap.isOpened()) {
      TRACE_INFO("Failed to open video filei\n");
      return false;
    }
