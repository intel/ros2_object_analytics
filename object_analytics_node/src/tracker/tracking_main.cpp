...
void show_usage()
{
  ...
  RCUTILS_LOG_INFO("tracker [-a algorithm] [-p dataset_path] [-t dataset_type] [-n dataset_name] [-h]\n");
  RCUTILS_LOG_INFO("options:\n");
  RCUTILS_LOG_INFO("-h : Print this help function.\n");
  RCUTILS_LOG_INFO("-a algorithm_name : Specify the tracking algorithm in the tracker.\n");
  RCUTILS_LOG_INFO("   supported algorithms: KCF,TLD,BOOSTING,MEDIAN_FLOW,MIL,GOTURN\n");
  RCUTILS_LOG_INFO("-p dataset_path : Specify the tracking datasets location.\n");
  RCUTILS_LOG_INFO("-t dataset_type : Specify the dataset type: video,image.\n");
...
{
public:
  explicit Streamer_node() : Node("streamer")
  {

    // Create a publisher with a custom Quality of Service profile.
    pub_2d_ = this-> create_publisher<sensor_msgs::msg::Image>(object_analytics_node::Const::kTopicRgb);

    pub_detected_objects_ = this->create_publisher<object_msgs::msg::ObjectsInBoxes>(
      object_analytics_node::Const::kTopicDetection);

    auto track_callback = [this](const typename object_analytics_msgs::msg::TrackedObjects::SharedPtr objs) -> void {
    this->track_cb(objs);
  };

    track_obj_ = create_subscription<object_analytics_msgs::msg::TrackedObjects>(object_analytics_node::Const::kTopicTracking, track_callback);
    ...
    auto autoplay =
      [this]() -> void
      {
		if(ds_->getNextFrame(frame_))
		{
			...
			sensor_msgs::msg::Image::SharedPtr image_br = std::make_shared<sensor_msgs::msg::Image>();

			cv_bridge::CvImage out_msg;
			builtin_interfaces::msg::Time stamp;
			stamp.nanosec = ds_->getFrameIdx()-1;
			out_msg.header.stamp = stamp;
			out_msg.header.frame_id = to_string(ds_->getFrameIdx()-1);
    ...
    auto autoDetect =
      [this]() -> void
      {
		if(((ds_->getFrameIdx() % 4) == 0) && (ds_->getFrameIdx() > 0))
		{
			...
			Rect2d roi = ds_->getIdxGT(frameId);
			...
			objs_in_boxes->header.frame_id =to_string(frameId);
    ...
    timerPlay_ = this->create_wall_timer(33ms, autoplay);
	timerPlay_->cancel();
    timerDetect_ = this->create_wall_timer(35ms, autoDetect);
	timerDetect_->cancel();
  }
  ...
  void track_cb(const object_analytics_msgs::msg::TrackedObjects::SharedPtr& objs);

  void initialDataset(string path, dsType type, string dsName)
  {
	ds_ = ds_->create(type);
	ds_->load(path);
  ...
  {
    frame.copyTo(image_);
    rectangle(image_, ds_->getIdxGT(ds_->getFrameIdx()), gtColor, 2, cv::LINE_8);
  ...
  void statu()
  {
	...
	RCUTILS_LOG_DEBUG("\r---------------------Overall status report-----------------------\n");
	RCUTILS_LOG_DEBUG("Overlap > 0 count(%d)\n", num_corr_);
	RCUTILS_LOG_DEBUG("Overlap > 0.5 count(%d)\n", num_corr_thd_);
	RCUTILS_LOG_DEBUG("present NUM(%d), response NUM(%d)\n", num_present_, num_response_);

	double precision = (double)num_corr_thd_/num_response_;
	double recall = (double)num_corr_thd_/num_present_;
	RCUTILS_LOG_DEBUG("Precision(%lf), recall(%lf)\n", precision, recall);
	RCUTILS_LOG_DEBUG("------------------------------End----------------------------------\n");

  }

protected:
  const string window = "Tracking API";
  const Scalar gtColor = Scalar(0, 255, 0);
  Ptr <trDataset> ds_;
...
private:
  ...
  rclcpp::Subscription<object_analytics_msgs::msg::TrackedObjects>::SharedPtr track_obj_;

};

void Streamer_node::track_cb(const object_analytics_msgs::msg::TrackedObjects::SharedPtr& objs)
{
	...
	for(auto t : objs->tracked_objects) {
		...
		Rect2d gt_roi = ds_->getIdxGT(frame_id);
		Rect2d obj_roi(t.roi.x_offset, t.roi.y_offset, t.roi.width, t.roi.height);
		...
		cv::Rect2d track_rect(t.roi.x_offset,t.roi.y_offset,t.roi.width,t.roi.height);
...
int main(int argc, char * argv[])
{
  ...
  // Create track node.
  auto r_node = std::make_shared<object_analytics_node::tracker::TrackingNode>( );
  r_node->setAlgo(algo);
