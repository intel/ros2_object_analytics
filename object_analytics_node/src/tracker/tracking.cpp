...
void Tracking::rectifyTracker(const std::shared_ptr<sFrame> frame,
...
void Tracking::updateTracker(const std::shared_ptr<sFrame> frame,
  ...
  if (det) {
    ...
    if (!ret) {
      TRACE_INFO(
          "Tracker(%d) update stamp(%f), det(%d), \
          failed since of no base frame!!!!",
...
void Tracking::storeTraj(timespec stamp, cv::Rect rect, cv::Mat& cov,
                         cv::Mat frame) {
