...
class SynchronizedSubscriber(Node):
    ...
    def __init__(self):
     ...
     """
        ...
        loc_sub = message_filters.Subscriber(self, ObjectsInBoxes3D, '/object_analytics/localization')
        det_sub = message_filters.Subscriber(self, ObjectsInBoxes, '/movidius_ncs_stream/detected_objects')
        tra_sub = message_filters.Subscriber(self, TrackedObjects, '/object_analytics/tracking')

        ts = message_filters.ApproximateTimeSynchronizer(
                   [det_sub, loc_sub, tra_sub], 10, 0.01)
    ...
    def _merge_results(self, detected_objects, localized_objects, tracked_objects):
        ...
        for obj in tracked_objects.tracked_objects:
            ...
            #for obj in tracked_objects.tracked_objects:
                #key = Roi(obj.roi)
            self.get_logger().info('[offset 1]: "%s, %s"' % (obj.roi.x_offset, obj.roi.y_offset))
            self.get_logger().info('[offset 2]: "%s, %s"' % (localized_map[key].roi.x_offset, localized_map[key].roi.y_offset))
